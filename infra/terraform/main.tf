locals {
  ...
  normalized_env     = lower(regexreplace(var.environment, "[^A-Za-z0-9]", ""))
...
resource "azurerm_linux_web_app" "api" {
  ...
  app_settings = {
    ...
    "DATABASE_URL" = format(
      ...
      format("%ssecrets/%s", azurerm_key_vault.main.vault_uri, var.database_url_secret_name)
    )
    "SECRET_KEY" = format(
      "@Microsoft.KeyVault(SecretUri=%s)",
      format("%ssecrets/%s", azurerm_key_vault.main.vault_uri, var.app_secret_key_secret_name)
    )
